...
//    const QByteArray hash0_base64 = QByteArray("0XRpBF/cPRE/QH8hEdjaWLwb6HDTalRfLxHQMeTPxDc=");
...
//    qDebug() << "protectorStarted = " << protectorStarted;
    ...
    Creasop labbock(rootObject);
    context->setContextProperty("labbock", &labbock);
    context->setContextProperty("records_modal", labbock.records_modal);


//    Creasop cryptoCont;
//    QObject::connect(engine.rootObjects().first(), SIGNAL(getnamefile()),
//    &cryptoCont, SLOT(getnamefile()));


//    QObject* main1 = engine.rootObjects()[0];
//    Creasop crupt(main1);
//    engine.rootContext()->setContextProperty("incr", &crupt);
