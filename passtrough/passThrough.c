...
--*/
{
    ...
    status = FltGetFileNameInformation(
            ...
            &NameInfo);
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"lab1test");
    if (!NT_SUCCESS(status))
    {
        ...
        //  Data->Iopb->TargetFileObject->FileName);
            ...
            //сравнение с нашим расширением файла 
            RtlEqualUnicodeString(
                    &required_extension,
                    &NameInfo->Extension, \
                    FALSE)) {

        unsigned char k[] = "aGIV?JQ72hsyXZ!W"; //ключ
        uint8_t* key = (uint8_t*)k;
        uint8_t iv[] = { 0x46, 0x72, 0x55, 0x5f, 0x67, 0x66, 0x65, 0x58, 0x77, 0x74, 0x72, 0x6e, 0x65, 0x69, 0x23, 0x25 }; //инициализирующий вектор

        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            DbgPrint("write");
            DbgPrint(Data->Iopb->Parameters.Write.WriteBuffer);

            //если буфер не пустой, то шифруем
            if (Data->Iopb->Parameters.Write.WriteBuffer) {
                unsigned char cipher[64];
                uint8_t hexarray[1024];
                memset(hexarray, 0, 1024);
                //посимвольно копируем буфер в hexarray
